export default class WorkTransform{
    ...
    _iniObject(){
        ...
        this.canvas.append(circle1,circle2)
    ...
    _beginAnim() {
        this.circle1.style.display = "flex"
        this.circle2.style.display = "flex"
    ...
    _action = () => {
        ...
        this.circle1.style.transform = `translate(${this.x[0]}px,${this.y[0]}px)`
        this.circle2.style.transform = `translate(${this.x[1]}px,${this.y[1] - 20}px)`

        if((this.x[0] - this.x[1]<=15) && (this.y[0] - this.y[1]<=15) && (this.x[0] - this.x[1]>=0) && (this.y[0] - this.y[1]>=0)) {
        ...
        for(let i = 0; i <this.x.length; i++) {
            ...
            if(this.x[i] + 1 * this.deg[i] + 10 * (3*i+2) > this.canvas.clientWidth-10 && this.direction[i][0] == 1) {
                this.direction[i][0] *= -1
                this.y[i] += (this.x[i] - (this.canvas.clientWidth - 10 * (3*i+2))) / this.deg[1] * this.direction[i][1]
                this.x[i] = this.canvas.clientWidth - 10 * (3*i+2)
            ...
            } else if(this.y[i] + 10*(3*i+2) > this.canvas.clientHeight - 5 && this.direction[i][1] == 1) {
                this.direction[i][1] = -1
                this.y[i] = this.canvas.clientHeight - 10 * (3*i+2) - 5
